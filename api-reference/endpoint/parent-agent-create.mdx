...
<Note>
**Financial Control**: Parent agents define spend rules and guardrails that control how child agents can process payments, providing centralized financial governance.
</Note>

<Info>
**Complete Control**: Configure transaction limits, approval requirements, and allowed payment rails to precisely control how your agents handle funds.
</Info>
...
<ParamField body="spend_rules" type="object" required>
  ...
  <CardGroup cols={2}>
    <Card title="Financial Controls" icon="calculator" color="#FF7A00">
      Set limits, thresholds, and allowances
    </Card>
    <Card title="Approval Workflows" icon="user-check" color="#FF7A00">
...
<Warning>
**Security Reminder**: Store the returned agent ID securely as it will be needed for creating child agents and processing payments.
</Warning>

<AccordionGroup>
  <Accordion title="Payment Rails Explained" icon="info">
    - **BANK_ACCOUNT**: All bank transfers
    - **BANK_ACCOUNT:ACH**: US bank transfers only
    - **BANK_ACCOUNT:SEPA**: European bank transfers only
    - **BANK_ACCOUNT:IMPS**: Indian bank transfers only
    - **MAGE_IDS**: Transfers to other Mage users/agents
    - **CHECKOUT**: Checkout link payments
    - **ALL**: Allow all payment types
  </Accordion>
</AccordionGroup>